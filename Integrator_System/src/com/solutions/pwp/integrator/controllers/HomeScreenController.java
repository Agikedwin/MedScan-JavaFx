...
public class HomeScreenController implements Initializable {
    ...
    private void init() {
        ...
        System.out.println("USE: "+FunctionGetUserDetails.getFullName(USER_PROFILE));
       //loggedIn.setText(FunctionGetUserDetails.getFullName(USER_PROFILE) + "  Logged in as  " + FunctionGetUserDetails.getUserGroupName(Integer.parseInt(FunctionGetUserDetails.getUserGroup(USER_PROFILE))));
        posImg.setImage(new Image("/com/solutions/pwp/integrator/icons/medscanicon.jpg"));
...
//new PWPTablesCreator();
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/views/MainUI.fxml"));
...
//            JPanel jpanelList[]= {jPanelApp1,jPanelApp2,jPanelApp3,jPanelApp4};
            ...
            String theAppList[] = {"MEDSCAN MIS", "ACCESS CONTROL"};
            ArrayList<String> systemApp = new ArrayList();

            int usergroup;
            resultSet = statement.executeQuery("select groupid from " + ASSIGNUSER_TOGROUP + " where userid='" + USER_PROFILE + "' ");
            if (resultSet.next()) {
